...
//DSPd SCI模块数据校验格式，注意寄存器限制的原因，一次只能获取一个字节
    RCBUF[ReciveRCOUNT++] =UARTCharGet(UART1_BASE); //&0x00FF接收到的八位数据；按照8位8位来进行接收数据；
//前4帧都是包头FE FE FE FE
        ...
        if(TXCOUNT==0) //发送队列为空
        {
            if(flagRC==1)//接收队列为空,表示接受队列已完成，接收到的数据包完全正确
            {
				...
				if(PackLength==19)//包长是否等于19，判断是否为粒子群反馈
				{
                    ...
                    //IPC传递
                    i = 0;
                    usMBuffer[i]=PackLength;
                    for (i=1; i< PackLength; i++)
                    {
                        usMBuffer[i]=RC_DataBUF[i-1];
                    }
...
void SciSend(void)
{
    if(flagSEND == 1)
    {
        ...
        if(TXCOUNT < SendDataNumber)
        {
            ...
            UARTCharPutNonBlocking(UART1_BASE, TXBUF[TXCOUNT]);
            TXCOUNT++;
        }
        else//一组数据已经发送结束
        {
    ...
    if(PSOSENDF)//经过了0.05s后，发送一次数据
    {
        ...
        else
        {
            if(PSOCOUNT < PSONumber)
            {
                ...
                UARTCharPutNonBlocking(UART1_BASE,PSOBUF[PSOCOUNT]);
                PSOCOUNT++;
            }
            else
            {
