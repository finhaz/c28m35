...
{
    if(SciaRegs.SCIRXST.bit.FE == 1)//有帧错误//这段话的注释除了这句话，不要相信
    {
        InitSci();
    }
    else
    ...
    //DSPd SCI模块数据校验格式，注意寄存器限制的原因，一次只能获取一个字节
        RCBUF[ReciveRCOUNT++] = SciaRegs.SCIRXBUF.bit.RXDT; //&0x00FF接收到的八位数据；按照8位8位来进行接收数据；
    //前4帧都是包头FE FE FE FE
...
void SciSend(void)
{
    if( SciaRegs.SCICTL2.bit.TXRDY == 1)  //发送器的缓冲器和移位寄存器都为空，每次发送前都要确认一下这个寄存器的标志位为1
    {
        if(flagSEND == 1)
        {
            ...
            {
                SciaRegs.SCITXBUF = TXBUF[TXCOUNT] ;//& 0X00FF;//发送，并再次确认为8位数据，这个寄存器就是用来发送字节的
                TXCOUNT++;
            }
            else//一组数据已经发送结束
            {
...
{
	if( SciaRegs.SCICTL2.bit.TXRDY == 1)//发送前都要确认一下这个寄存器的标志位为1
	{
		if(PSOSENDF)//经过了0.05s后，发送一次数据
		{
			...
			else
			{
				...
				{
					SciaRegs.SCITXBUF = PSOBUF[PSOCOUNT] ;//& 0X00FF;//发送，并再次确认为8位数据，这个寄存器就是用来发送字节的
					PSOCOUNT++;
				}
				else
				{
