...
Uint16 Adc1Conversion(void)
{
    ...
    while (Adc1Regs.ADCSOCFLG1.all != 0) {}  //Wait for pending SOCs to complete
    ...
    //Reset RR pointer so that next SOC is SOC0
    Adc1Regs.SOCPRICTL.bit.SOCPRIORITY = 1;
    Adc1Regs.SOCPRICTL.bit.SOCPRIORITY = 0;
...
Uint16 Adc2Conversion(void)
{
    ...
    while (Adc2Regs.ADCSOCFLG1.all != 0) {}  //Wait for pending SOCs to complete

    //Clear any pending interrupts
    Adc2Regs.ADCINTFLGCLR.bit.ADCINT1 = 1;
    Adc2Regs.ADCINTFLGCLR.bit.ADCINT2 = 1;
    Adc2Regs.ADCINTOVFCLR.bit.ADCINT1 = 1;
    Adc2Regs.ADCINTOVFCLR.bit.ADCINT2 = 1;

    //Reset RR pointer so that next SOC is SOC0
    Adc2Regs.SOCPRICTL.bit.SOCPRIORITY = 1;
    Adc2Regs.SOCPRICTL.bit.SOCPRIORITY = 0;
