//###########################################################################
// FILE:   F28M35x_SWPiroritizedPieVect.c
// TITLE:  F28M35x Devices SW Prioritized PIE Vector Table Initialization.
//###########################################################################
...
// Group 11 PIE Vectors
     ...
     #if (G111PL != 0)
    MTOCIPC_INT1_ISR,      // Master to Control Processor
     #else
    INT_NOTUSED_ISR,
     #endif

     #if (G112PL != 0)
    MTOCIPC_INT2_ISR,      // Master to Control Processor
     #else
    INT_NOTUSED_ISR,
     #endif

     #if (G113PL != 0)
    MTOCIPC_INT3_ISR,      // Master to Control Processor
     #else
    INT_NOTUSED_ISR,
     #endif

     #if (G114PL != 0)
    MTOCIPC_INT4_ISR,      // Master to Control Processor
     #else
    INT_NOTUSED_ISR,
     #endif
...
// Group 12 PIE Vectors
    ...
    INT_NOTUSED_ISR,
     ...
     #if (G122PL != 0)
    CFLSINGERR_ISR,     // Flash Single Error
     #else
    INT_NOTUSED_ISR,
     #endif
    ...
    INT_NOTUSED_ISR,
     ...
     #if (G124PL != 0)
    CRAMSINGERR_ISR,    // RAM Single Error
     #else
    INT_NOTUSED_ISR,
     #endif

     #if (G125PL != 0)
    CRAMACCVIOL_ISR,    // RAM Access Violation
     #else
    INT_NOTUSED_ISR,
     #endif
