//###########################################################################
// FILE:    F28M35x_SWPrioritizedDefaultIsr.c
// TITLE:   F28M35x Device Default Software Prioritized Interrupt Service
//          Routines.
//###########################################################################
...
#include "F28M35x_SWPrioritizedIsrLevels.h"
...
#if (G53PL != 0)
__interrupt void EQEP3_INT_ISR(void)     // eQEP3 Interrupt
{
    // Set interrupt priority:
    volatile Uint16 TempPIEIER = PieCtrlRegs.PIEIER5.all;
    IER |= M_INT5;
...
#if (G58PL != 0)
__interrupt void EPWM9_INT_ISR(void)     // ePWM9 Interrupt
{
    // Set interrupt priority:
    volatile Uint16 TempPIEIER = PieCtrlRegs.PIEIER5.all;
    IER |= M_INT5;
...
__interrupt void DINTCH2_ISR(void)     // DMA Channel 2
{
    ...
    IER |= M_INT7;
    IER &= MINT7;                      // Set "global" priority
    PieCtrlRegs.PIEIER6.all &= MG72;   // Set "group"  priority
    PieCtrlRegs.PIEACK.all = 0xFFFF;   // Enable PIE interrupts
    EINT;
...
__interrupt void DINTCH3_ISR(void)     // DMA Channel 3
{
    ...
    IER |= M_INT7;
    IER &= MINT7;                      // Set "global" priority
    PieCtrlRegs.PIEIER6.all &= MG73;   // Set "group"  priority
    PieCtrlRegs.PIEACK.all = 0xFFFF;   // Enable PIE interrupts
    EINT;
...
__interrupt void DINTCH4_ISR(void)     // DMA Channel 4
{
    ...
    IER |= M_INT7;
    IER &= MINT7;                      // Set "global" priority
    PieCtrlRegs.PIEIER6.all &= MG74;   // Set "group"  priority
    PieCtrlRegs.PIEACK.all = 0xFFFF;   // Enable PIE interrupts
    EINT;
...
// Connected to PIEIER7_1 (use MINT7 and MG75 masks):
#if (G75PL != 0)
__interrupt void DINTCH5_ISR(void)     // DMA Channel 5
{
    ...
    IER |= M_INT7;
    IER &= MINT7;                      // Set "global" priority
    PieCtrlRegs.PIEIER6.all &= MG75;   // Set "group"  priority
    PieCtrlRegs.PIEACK.all = 0xFFFF;   // Enable PIE interrupts
    EINT;
...
__interrupt void DINTCH6_ISR(void)     // DMA Channel 6
{
    ...
    IER |= M_INT7;
    IER &= MINT7;                      // Set "global" priority
    PieCtrlRegs.PIEIER6.all &= MG76;   // Set "group"  priority
    PieCtrlRegs.PIEACK.all = 0xFFFF;   // Enable PIE interrupts
    EINT;
...
#if (G111PL != 0)
__interrupt void  MTOCIPC_INT1_ISR(void)      // Master to Control IPC Interrupt
{
    // Set interrupt priority:
...
#if (G112PL != 0)
__interrupt void  MTOCIPC_INT2_ISR(void)      // Master to Control IPC Interrupt
{
    // Set interrupt priority:
    volatile Uint16 TempPIEIER = PieCtrlRegs.PIEIER11.all;
    IER |= MINT11;                     // Set "global" priority
...
#if (G113PL != 0)
__interrupt void  MTOCIPC_INT3_ISR(void)      // Master to Control IPC Interrupt
{
    // Set interrupt priority:
    volatile Uint16 TempPIEIER = PieCtrlRegs.PIEIER11.all;
    IER |= MINT11;                     // Set "global" priority
...
#if (G114PL != 0)
__interrupt void  MTOCIPC_INT4_ISR(void)      // Master to Control IPC Interrupt
{
    // Set interrupt priority:
    volatile Uint16 TempPIEIER = PieCtrlRegs.PIEIER11.all;
    IER |= MINT11;                     // Set "global" priority
...
// Connected to PIEIER12_1 (use MINT12 and MG122 masks):
#if (G122PL != 0)
...
#if (G123PL != 0)
__interrupt void CFLFSM_ISR(void)     // Flash FSM Interrupt
{
    // Set interrupt priority:
    volatile Uint16 TempPIEIER = PieCtrlRegs.PIEIER12.all;
    IER |= M_INT12;
