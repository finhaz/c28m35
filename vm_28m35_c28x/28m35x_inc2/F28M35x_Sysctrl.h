...
struct CXCLK_BITS {                    // bits description
        Uint16 XCLKOUTDIV : 2;         // 1:0    CVLKOUT Divider
        Uint16 rsvd2 : 14;             // 15:2   Reserved
};

union CXCLK_REG {
        Uint16 all;
        struct    CXCLK_BITS bit;
};
...
struct SYS_CTRL_REGS {
        union    CXCLK_REG CXCLK;         // XCLOCKOUT Divider Register
        Uint16 rsvd1[1];                  // Reserved
        union    CLKCTL_REG CLKCTL;       // CPU Timer 2 Clock Configuration
                                          // Register
